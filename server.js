...
STAKE_TIERS.forEach(stake => {
  rooms[stake] = { players: new Map(), waveEndsAt: Date.now() + WAVE_DURATION, waveNumber: 1 };
});
...
function makeSnake(id, name, stake) {
  ...
  return {
    ...
    angle: 0, targetAngle: 0, speed: BASE_SPEED,
    boost: BOOST_MAX, boosting: false,
    alive: true, sessionEnded: false,
    kills: 0, earnedTon: 0, joinedAt: Date.now(),
  };
}
...
function endSession(room, p, reason) {
  ...
  io.to(`room-${p.id}-private`).emit('session_end', {
    reason, kills: p.kills, earnedTon: p.earnedTon, pnl, length: p.seg.length,
  });
  if (reason === 'death') {
...
function tickRoom(stake) {
  ...
  if (waveJustEnded) {
    const board = livePlayers.filter(p => p.alive)
      .sort((a, b) => b.earnedTon - a.earnedTon).slice(0, 5)
      .map(p => ({ name: p.name, earnedTon: p.earnedTon, kills: p.kills, length: p.seg.length }));
    io.to(`room-${stake}`).emit('wave_end', { waveNumber: room.waveNumber, leaderboard: board, nextWaveEndsAt: room.waveEndsAt });
  }
}
