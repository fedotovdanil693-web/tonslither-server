...
function makeSnake(id, name, stake) {
  ...
  return {
    ...
    joinedAt: Date.now(),
    get width() { return 9 + Math.min(this.kills * 3, 18); },
    get targetLen() { return START_SEGS + this.kills * 20; },
  };
}
...
function moveSnake(p, dt) {
  ...
  while (p.seg.length > p.targetLen) p.seg.pop();
}
  ...
  for (const p of players) {
    ...
    for (const other of players) {
      ...
      for (let i = 5; i < other.seg.length; i += 2) {
        if (dist(head, other.seg[i]) < other.width * 0.85) { hitBody = true; break; }
      }
      if (hitBody) { killPlayer(room, p, other); break; }

      // head-on
      if (dist(head, oh) < (hw + other.width) * 0.5) {
